...
version=LGT_Devil-$TARGET-$MODE-0.37.5_$(date +%Y%m%d)
...
# Copy Kernel Image
rm -f $KERNEL_PATH/releasetools-lgt/$CUSTOM_PATH/tar/$KBUILD_BUILD_VERSION.tar
rm -f $KERNEL_PATH/releasetools-lgt/$CUSTOM_PATH/zip/$KBUILD_BUILD_VERSION.zip
...
cp boot.img $KERNEL_PATH/releasetools-lgt/$CUSTOM_PATH/tar
...
rm $KERNEL_PATH/releasetools-lgt/$CUSTOM_PATH/tar/boot.img
